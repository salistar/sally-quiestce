...
export default function LoginScreen() {
  ...
  const { promptAsync, ready: googleReady, missingClientId, missingNative } = useGoogleSignIn(
    async (idToken: string) => {
      setLoading(true);
      try {
        await api.loginWithGoogle(idToken, { gameType: 'quiestce' });
        console.log('[Quiestce/Login] Google login successful');
        router.replace('/(tabs)');
      } catch (e: any) {
        console.error('[Quiestce/Login] Google error:', e);
        Alert.alert(t('error'), e.message || 'Google sign-in failed');
      } finally {
        setLoading(false);
      }
    },
    (msg: string) => { console.warn('[Quiestce/Login] Google:', msg); setLoading(false); }
  );
